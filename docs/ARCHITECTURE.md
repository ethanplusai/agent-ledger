...
## Optional project answers

`lens/assistant.py` prepares fixed project-scoped evidence packets from FTS matches, nearby messages, and project notes. HTTP preparation limits query time; packets/results are capped at eight and expire on subsequent preparation after 30 minutes. A separate authenticated start action invokes the installed Claude CLI in an empty temporary working directory, with input over stdin and no shell interpolation. Tools, MCP, skills, hooks, and session persistence are disabled through checked CLI flags. This is an application boundary, not an OS sandbox; the CLI retains access to its authentication and configured provider.

A worker runs outside the SQLite lock. One answer runs at a time, with a 120-second deadline, 1 MiB stdout ceiling, cancellation, and shutdown cleanup. Output and prompts receive best-effort secret scrubbing. Demo mode uses an explicit synthetic answer and never calls a model. The UI links only citations in the supplied packet; it warns on missing or unknown citations, and leaves factual verification to the reader. All memory edits remain explicit note mutations.
